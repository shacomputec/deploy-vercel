...
const SECTIONS: Section[] = [
  ...
  {
    id: "data-formats",
    title: "Phone & ID formats (validated fields)",
    icon: IdCardIcon,
    intro:
      "Phone numbers and Ghana Card numbers are VALIDATED everywhere in the system — on the website, the Windows desktop app, the Android app and the result checker. The form tells you the exact problem before you save, and the server refuses anything that doesn't match. This cheat-sheet shows the only accepted formats, so office staff enter clean data the first time.",
    steps: [
      "PHONE NUMBERS — Ghana phone numbers are 10 digits starting with 0 (e.g. 0241234567). Type them in any of these forms and the system normalizes them automatically:",
      "   • 0241234567        (10 digits — the standard form)",
      "   • 0244 000 000      (spaces are stripped)",
      "   • 0244-000-000      (dashes are stripped)",
      "   • +233241234567     (international form is accepted and stored as-is)",
      "   • 233241234567      (also accepted)",
      "REJECTED examples: 024123456 (9 digits), 1234567890 (does not start with 0 / +233), 055-123-45 (too short), abc123 (not a number).",
      "GHANA CARD NUMBERS — the official GHA format, three segments joined by hyphens: the GHA- country prefix, 9 system-generated digits, and a 1-digit check digit:",
      "   • GHA-123456789-0   (the full, correct format — GHA- + 9 digits + check digit)",
      "   • gha123456789-0    (typed lowercase without the dash → stored as GHA-123456789-0)",
      "REJECTED examples: GHA-123456789 (missing the check digit), GHA-1234567890 (10 digits — the check digit must be separated by a hyphen), GHA-1234 (too few digits), 123456789-0 (no GHA- prefix).",
      "NHIS NUMBERS (National Health Insurance) — exactly 9 numeric digits, no prefix, no letters:",
      "   • 123456789         (the only accepted form — 9 digits)",
      "   • 123 456 789       (spaces/dashes are stripped automatically)",
      "REJECTED examples: 12345678 (8 digits), 1234567890 (10 digits), GHA-123456789 (that is a Ghana Card format), 12345678A (contains a letter).",
      "WHERE THIS APPLIES — students, parents, teachers, staff, users, school profile, online admissions, the contact form, and the result-checker OTP (the phone entered must be the registered 10-digit number — +233 and 0 forms match the same record).",
      "Tip: if a phone is rejected, it is missing a digit, starts with the wrong prefix, or has letters — fix the format rather than trying to force the save.",
    ],
    notes: [
      "The same rules are enforced on every device: web, desktop and mobile share one server, so a phone that saves on the website is valid everywhere.",
      "Admission numbers (auto-generated, e.g. GES-2027-0001) and weighing-card numbers are separate fields and follow their own formats — the Ghana Card rule applies only to the Ghana Card field, and the NHIS rule only to the NHIS field.",
    ],
  },
  {
    id: "dashboard",
  ...
  },
  {
    ...
    steps: [
      ...
      "Open a card's Comments and click “AI comment” — the system drafts a warm, personalised remark from the student's actual scores (works offline with the built-in shacomputec AI writer; connect your own AI key for smarter drafts).",
    ],
    notes: [
